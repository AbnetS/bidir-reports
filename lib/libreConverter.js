...
exports.convert = (document, format, filter, callback) => {
    ...
    return async.auto({
        soffice: (callback) => {
            let paths = [];
            pathsToReturn = process.platform;
            switch (process.platform) {
                case 'darwin': paths = ['/Applications/LibreOffice.app/Contents/MacOS/soffice'];
                    break;
                case 'linux': paths = ['/usr/bin/libreoffice', '/usr/bin/soffice', '/lib/libreoffice/program/soffice']
                    break;
                case 'win32': paths = [
        ...
        //return callback(null, pathsToReturn);
    });
};
